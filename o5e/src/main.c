...
void main( void )
{
    ...
    if ( Flash_OK ) {        // don't run these with nonsense flash values 
        (void)task_create(Engine10_Task, 0 + 128, 0, 0, 0);
        (void)task_create(Fuel_Pump_Task, 1 + 128, 0, 0, 0);
        (void)task_create(Slow_Vars_Task, 2 + 128, 0, 0, 0);
//mcb        (void)task_create(Eng_Pos_Status_BLINK_Task, 3 + 128, 0, 0, 0);
        
        if (Sync_Mode_Select == 1){   // user option to generate a cam signal
           (void)task_create(Cam_Pulse_Task, 4 + 128, 0, 0, 0);
        }
        
        //if (Generic_Output_1_type != 0){  //optional user configured output
        (void)task_create(Generic_Output_1_Task, 5 + 128, 0, 0, 0);	
        //}

        //if (Generic_Output_2_type != 0){  //optional user configured output
       // (void)task_create(Generic_Output_2_Task, 6 + 128, 0, 0, 0);	
       // }                
        //if (Generic_Output_3_type != 0){  //optional user configured output
        //(void)task_create(Generic_Output_3_Task, 7 + 128, 0, 0, 0);	
        //}
        //if (Generic_Output_4_type != 0){  //optional user configured output
        //(void)task_create(Generic_Output_4_Task, 8 + 128, 0, 0, 0);	
        //}       
        
    } 
    
   	(void)task_create(Test_RPM_Task, 9 + 128, 0, 0, 0);
    (void)task_create(Tuner_Task, 10 + 128, 0, 0, 0);
//mcb    (void)task_create(LED_Task, 11 + 128, 0, 0, 0);

    (void)task_create(Angle_Clock_Task, 254, 0, 0, 0);         // update angle clock, always last
...
static void Angle_Clock_Task(void)
{
    static uint32_t i, prev_angle; // previous cam position in ticks 
    register uint64_t j =0;
    static int8_t Previous_Status, status;
    static uint32_t Start_Degrees, Start_Time;     // start started
...
// Task to verify that OS is running - flash a LED, feed TunerStudio
static void LED_Task(void)
{

    task_open();                        // NOTE:  no local variables allowed
    task_wait(410);                     // wait???


    for (;;) {
        
        if (EDMA.ESR.R != 0)            // DEBUG - check for DMA errors, if we have DMA errors, so what ?
            err_push( CODE_OLDJUNK_D5 );

        led_invert( LED1 );             
        if ( Last_Error == 0 )          // flash every second, blink faster for errors
            task_wait(200);             // 999, 303
        else
            task_wait(75);
