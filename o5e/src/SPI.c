...
******************************************************************************/
static unsigned char *pInData = ((unsigned char *)&SIU.GPDO[3].R) + 1;         /*!< Pointer 
                                                                                * to the input data accessing parallel GPIO registers. */
static unsigned char *pOutData = ((unsigned char *)&SIU.GPDO[3].R) + 2;        /*!< Pointer 
...
static void SpiInit01(void)
{
    ...
    DSPI_C.MCR.R = 0x01013C00;  /* SPI, Slave (CS0 incative high), disable FIFOs */
    DSPI_C.MCR.B.DIS_TXF = 1;   /* disable FIFOs */
    DSPI_C.MCR.B.DIS_RXF = 1;
    DSPI_C.CTAR[0].R = 0x38000000;      /* 8 bit, CPHA,CPOL= 0,0 */

    DSPI_B.MCR.R = 0x01013C00;  /* SPI, Slave (CS0 incative high), disable FIFOs */
    DSPI_B.MCR.B.DIS_TXF = 1;   /* disable FIFOs */
    DSPI_B.MCR.B.DIS_RXF = 1;
    DSPI_B.CTAR[0].R = 0x3A000000;      /* 8 bit, CPHA,CPOL= 1,0 */
...
static void SpiRun01(void)
{
    ...
    for (i = 0; i < 12; i++) {
        ...
        DSPI_C.PUSHR.R = (unsigned long)getData[i];     /* on the slave side - DSPI */
        *pOutData = putData[i]; /* on the master side - DMA */
        ...
        /* retrieving the data */
        slaveGot[i] = (unsigned char)DSPI_C.POPR.R;     /* what was actually received on DSPI slave */
