...
{  
  ...
  if (Load_Sense <= 3){
      Reference_VE = (uint16_t)MAP[1];//  using MAP 2 until angle reading fixed on MAP1 (MAP[0])
      ...
      Reference_VE = (uint16_t)(((MAF[0] * 60)<<4) / gram_flow) ; //get %VE and convert back to bin 12
