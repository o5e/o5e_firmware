...
main()
{
    static unsigned char buf[85];

    buf[0] = 0x06;
    buf[1] = 0x3A;
    buf[2] = 0x02;
    buf[9] = 0x03;
// x06 x3A x02 x00 x00 x00 x00 x00 x00 x03 x00 x00 x00 x00 x00 x00  

    printf("%x", Crc32_ComputeBuf(0, buf, 85));
}
#endif
...
main()
{
    ...
    result = Crc32_ComputeBuf(0, buf, 85);
    printf("result = %x\n", result);

    crcInit();
    result = crcFast((unsigned char *)buf, 85);
    printf("result = %x\n", result);

    result = crcSlow((unsigned char *)buf, 85);
    printf("result = %x\n", result);
}
